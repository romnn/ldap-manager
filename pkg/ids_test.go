...
func TestGetHighestGID(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("failed to get highest GID: %v", err)
	}
	if GID != MinGID+2 {
		t.Fatalf("expected GID %d but got %d", MinGID+2, GID)
	}
	...
	if group.GetGID() != MinGID+2 {
		t.Fatalf("expected GID %d but got %d", MinGID+2, group.GetGID())
	}
	...
	if GID != MinGID+3 {
		t.Fatalf("expected GID %d but got %d", MinGID+3, group.GetGID())
	}
}
...
func TestGetHighestUID(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("failed to get highest UID: %v", err)
	}
	if UID != MinUID+1 {
		t.Fatalf("expected UID %d but got %d", MinUID+1, UID)
	}
	...
	if err != nil {
		t.Fatalf("failed to get user %q: %v", username, err)
	}
	t.Log(PrettyPrint(user))
	if user.GetUID() != MinUID+1 {
		t.Fatalf("expected UID %d but got %d", MinUID+1, user.GetUID())
	}
	...
	if UID != MinUID+2 {
		t.Fatalf("expected UID %d but got %d", MinUID+2, UID)
	}
}
